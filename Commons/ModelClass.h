...
public:
	...
	// Model을 이루는 모든 meshes를 순회하며 Draw() 호출
	void Draw(Shader& shader)
	{
		for (unsigned int i = 0; i < meshes.size(); i++)
			meshes[i].Draw(shader); //배열 안의 i번째 Mesh 객체 내부의 Draw함수 호출
	}
...
private:
	...
	void processNode(aiNode* node, const aiScene* scene)
	{
		...
		for (unsigned int i = 0; i < node->mNumMeshes; i++)
		{
			...
			// 찾아낸 aiMesh포인터를 processMesh로 전달
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		// loadMaterialTextures 함수로 이미지(diffuse,specular)를 불러와 textures배열에 합침(insert)
